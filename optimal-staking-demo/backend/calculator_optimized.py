...
def calculate_tracking_error(request: Union[CalculationRequest, dict]) -> CalculationResponse:
    ...
    opt_calc = create_optimized_calculator(
        benchmark_weights, market_params, redemption_dist, 
        yield_params, unbonding_days, fund_details
    )
    ...
    benchmark = BenchmarkWeights() if not request.market else BenchmarkWeights(
    ...
    market = MarketParameters() if not request.market else MarketParameters(
    ...
    sensitivity_points_2d = perform_sensitivity_analysis_2d_optimized(
        opt_calc
    )
...
def perform_sensitivity_analysis_optimized(
    ...
    staking_levels = np.array([0.70, 0.75, 0.80, 0.85, 0.88, 0.89, 0.90, 
    ...
    results = opt_calc.calculate_net_benefits(tracking_errors, staking_levels, staking_levels)
...
def perform_sensitivity_analysis_2d_optimized(
    ...
    for i in range(n_points):
        for j in range(n_points):
            sensitivity_points_2d.append(SensitivityPoint2D(
