...
def convert_legacy_request(legacy_req: LegacyCalculationRequest) -> CalculationRequest:
    ...
    distribution = [
        RedemptionDistributionItem(size=size, probability=count / total_count)
...
def perform_sensitivity_analysis(
    ...
    min_baseline = min(
        yield_params.eth_baseline_staking_pct, yield_params.sol_baseline_staking_pct
    )
...
def perform_sensitivity_analysis_2d(
    ...
    The search space starts at each asset's baseline staking percentage,
    allowing optimization to recommend levels below the baseline when beneficial.
    """
    # Generate 1% increments starting from each asset's baseline
    eth_min_pct = int(yield_params.eth_baseline_staking_pct * 100)
    sol_min_pct = int(yield_params.sol_baseline_staking_pct * 100)

    eth_levels = [i / 100.0 for i in range(eth_min_pct, 101)]
    sol_levels = [i / 100.0 for i in range(sol_min_pct, 101)]
    results = []
