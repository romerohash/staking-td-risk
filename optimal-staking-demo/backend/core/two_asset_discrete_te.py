...
class BenchmarkWeights:
    ...
    btc_weight: float = Field(0.7869, description="BTC weight (78.69%)")
    eth_weight: float = Field(0.1049, description="ETH weight (10.49%)")
    xrp_weight: float = Field(0.0549, description="XRP weight (5.49%)")
    sol_weight: float = Field(0.0387, description="SOL weight (3.87%)")
    ada_weight: float = Field(0.0119, description="ADA weight (1.19%)")
    xlm_weight: float = Field(0.0027, description="XLM weight (0.27%)")
    ...
    def as_array(self) -> NDArray[np.float64]:
        """Return weights as numpy array in order: BTC, ETH, XRP, SOL, ADA, XLM"""
        return np.array([
            self.btc_weight, self.eth_weight, self.xrp_weight,
            self.sol_weight, self.ada_weight, self.xlm_weight
        ])
...
class MarketParameters:
    ...
    daily_volatilities: Dict[str, float] = field(default_factory=lambda: {
        'BTC': 0.039, 'ETH': 0.048, 'XRP': 0.053,
        'SOL': 0.071, 'ADA': 0.055, 'XLM': 0.051
    })
    correlations: Dict[Tuple[str, str], float] = field(default_factory=lambda: {
        ('BTC', 'ETH'): 0.70,
        ('BTC', 'XRP'): 0.60, ('BTC', 'SOL'): 0.60, ('BTC', 'ADA'): 0.60, ('BTC', 'XLM'): 0.60,
        ('ETH', 'XRP'): 0.60, ('ETH', 'SOL'): 0.60, ('ETH', 'ADA'): 0.60, ('ETH', 'XLM'): 0.60,
        ('XRP', 'SOL'): 0.60, ('XRP', 'ADA'): 0.60, ('XRP', 'XLM'): 0.60,
        ('SOL', 'ADA'): 0.60, ('SOL', 'XLM'): 0.60,
        ('ADA', 'XLM'): 0.60
    })
...
class RedemptionDistribution:
    ...
    redemption_sizes: List[float] = field(default_factory=lambda: [0.05, 0.10, 0.20, 0.30])
    redemption_counts: List[int] = field(default_factory=lambda: [12, 3, 2, 1])
    expected_redemptions_per_year: float = Field(18, description="Lambda - expected redemptions/year")
    ...
    def mean(self) -> float:
        ...
        return sum(size * prob for size, prob in zip(self.redemption_sizes, self.probabilities))
    
    @property
    def variance(self) -> float:
        """Variance of redemption size"""
        mean = self.mean
        return sum(prob * size**2 for size, prob in zip(self.redemption_sizes, self.probabilities)) - mean**2


@dataclass(frozen=True)
class StakingYieldParameters:
    """Parameters for staking yield calculations"""
    eth_annual_yield: float = Field(0.03, description="ETH annual staking yield (e.g., 3%)")
    sol_annual_yield: float = Field(0.073, description="SOL annual staking yield (e.g., 7.3%)")
    eth_baseline_staking_pct: float = Field(0.70, description="ETH reference staking level")
    sol_baseline_staking_pct: float = Field(0.70, description="SOL reference staking level")
    ...
    @field_validator('eth_annual_yield', 'sol_annual_yield', 'eth_baseline_staking_pct', 'sol_baseline_staking_pct')
...
class FundDetails:
    ...
    current_td: float = Field(0.0143, description="Current expected tracking difference")
...
class TwoAssetTrackingError:
    ...
    def _compute_k_components(self) -> Dict[str, float]:
        ...
        k_eth_sol = self.benchmark.eth_weight * self.benchmark.sol_weight * v_eth_sigma_v_sol
    ...
    def expected_values(self) -> Dict[str, float]:
        ...
        for r, p in zip(self.redemption.redemption_sizes, self.redemption.probabilities):
    ...
    def tracking_error(self) -> float:
        ...
        variance_days = (d_short * expectations['exp_var_full'] +
    ...
    def decompose_results(self) -> Dict[str, float]:
        ...
        eth_variance = lambda_r * d_long * self.k_components['k_eth_eth'] * expectations['exp_eth_squared']
        sol_variance = lambda_r * d_short * self.k_components['k_sol_sol'] * expectations['exp_sol_squared']
        cross_variance = lambda_r * d_short * 2 * self.k_components['k_eth_sol'] * expectations['exp_cross_term']
        ...
        return {
            ...
            'correlation_cost_pct': (te / independence_approx - 1) * 100 if independence_approx > 0 else 0,
            'eth_contribution_pct': eth_variance / total_variance * 100 if total_variance > 0 else 0,
            'sol_contribution_pct': sol_variance / total_variance * 100 if total_variance > 0 else 0,
            'cross_contribution_pct': cross_variance / total_variance * 100 if total_variance > 0 else 0
        }
    ...
    def net_benefit_analysis(self) -> Dict[str, float]:
        ...
        eth_benefit_baseline = self.benchmark.eth_weight * max(0, self.staking.eth_staking_pct - self.yield_params.eth_baseline_staking_pct) * self.yield_params.eth_annual_yield
        sol_benefit_baseline = self.benchmark.sol_weight * max(0, self.staking.sol_staking_pct - self.yield_params.sol_baseline_staking_pct) * self.yield_params.sol_annual_yield
        ...
        exp_excess_eth = sum(
            ...
            for r, prob in zip(self.redemption.redemption_sizes, self.redemption.probabilities)
        )
        exp_excess_sol = sum(
        ...
        eth_episode_fraction = self.redemption.expected_redemptions_per_year * self.staking.eth_unbonding_days / 365
        sol_episode_fraction = self.redemption.expected_redemptions_per_year * self.staking.sol_unbonding_days / 365
        
        eth_benefit_marginal = self.benchmark.eth_weight * self.yield_params.eth_annual_yield * eth_episode_fraction * exp_excess_eth
        sol_benefit_marginal = self.benchmark.sol_weight * self.yield_params.sol_annual_yield * sol_episode_fraction * exp_excess_sol
        ...
        tracking_difference_budget = self.fund_details.cap_td - self.fund_details.current_td
...
def main():
    ...
    benchmark = BenchmarkWeights()
    staking = StakingParameters(
        eth_staking_pct=0.90,
        sol_staking_pct=0.90
    )
    market = MarketParameters()
    redemption = RedemptionDistribution()
    yield_params = StakingYieldParameters(
    ...
    # Create calculator
    calculator = TwoAssetTrackingError(benchmark, staking, market, redemption, yield_params)
    ...
    print("\nStaking Parameters:")
    print(f"  ETH: {staking.eth_staking_pct:.0%} staked, {staking.eth_unbonding_days} day unbonding")
    print(f"  SOL: {staking.sol_staking_pct:.0%} staked, {staking.sol_unbonding_days} day unbonding")
    ...
    print(f"  Correlation cost: {results['correlation_cost']:.4%} ({results['correlation_cost_pct']:.1f}% increase)")
    ...
    print(f"Net Benefit: {benefits['net_benefit']:.3%} ({benefits['net_benefit_bps']:.1f} bps)")
    ...
    staking_levels = [0.70, 0.75, 0.80, 0.85, 0.88, 0.89, 0.90, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98]
    results = []
    
    for level in staking_levels:
        test_staking = StakingParameters(eth_staking_pct=level, sol_staking_pct=level)
        test_calc = TwoAssetTrackingError(benchmark, test_staking, market, redemption, yield_params)
    ...
    for i, (level, yield_benefit, te, shortfall, net_bps) in enumerate(results):
        ...
        print(f"{level:5.0%}{marker}    {yield_benefit:6.3%}    {te:6.3%}    {shortfall*10000:6.1f}    {net_bps:+6.1f}    {change:+5.1f}")
        prev_net = net_bps
    ...
    for eth_stake, sol_stake in scenarios:
        scenario_staking = StakingParameters(
            eth_staking_pct=eth_stake,
            sol_staking_pct=sol_stake
        )
        scenario_calc = TwoAssetTrackingError(benchmark, scenario_staking, market, redemption, yield_params)
        scenario_benefits = scenario_calc.net_benefit_analysis()
        
        marker = "*" if eth_stake == sol_stake and 0.92 <= eth_stake <= 0.94 else " "
        print(f"{eth_stake:.0%}/{sol_stake:.0%} {marker}        "
