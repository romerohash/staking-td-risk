...
def calculate_tracking_error_vectorized(
    ...
    for i in range(n_levels):
        ...
        for j in range(len(redemption_sizes)):
            ...
            var_full = (k_eth_eth * excess_eth * excess_eth +
...
def calculate_net_benefits_vectorized(
    ...
    for i in range(n_levels):
        ...
        eth_benefit_baseline = eth_weight * max(0.0, staking_levels_eth[i] - eth_baseline) * eth_yield
        sol_benefit_baseline = sol_weight * max(0.0, staking_levels_sol[i] - sol_baseline) * sol_yield
        ...
        eth_benefit_marginal = eth_weight * eth_yield * eth_episode_fraction * exp_excess_eth
        sol_benefit_marginal = sol_weight * sol_yield * sol_episode_fraction * exp_excess_sol
        
        total_benefit = (eth_benefit_baseline + eth_benefit_marginal + 
                        sol_benefit_baseline + sol_benefit_marginal)
        ...
        td_budget_deficit = min(0.0, tracking_difference_budget - abs(expected_shortfall))
    ...
    return yield_benefits, expected_shortfalls, tracking_difference_budgets, td_budget_deficits, net_benefits, net_benefits_bps
...
class OptimizedCalculator:
    ...
    def calculate_tracking_errors(
        ...
        return calculate_tracking_error_vectorized(
            staking_levels_eth, staking_levels_sol,
            k_eth_eth, k_sol_sol, k_eth_sol,
            self.redemption_sizes, self.redemption_probs,
            self.lambda_r, self.d_eth, self.d_sol
        )
    ...
    def calculate_net_benefits(
        ...
        yields, shortfalls, td_budgets, td_deficits, net_ben, net_bps = calculate_net_benefits_vectorized(
            tracking_errors, staking_levels_eth, staking_levels_sol,
            self.eth_weight, self.sol_weight,
            self.eth_yield, self.sol_yield,
            self.eth_baseline, self.sol_baseline,
            self.redemption_sizes, self.redemption_probs,
            self.lambda_r, self.d_eth, self.d_sol,
            self.current_td, self.cap_td
        )
