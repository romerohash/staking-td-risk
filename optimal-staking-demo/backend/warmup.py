...
def warmup_numba_functions():
    ...
    te_results = calculate_tracking_error_vectorized(
        staking_levels_eth, staking_levels_sol,
        k_eth_eth, k_sol_sol, k_eth_sol,
        redemption_sizes, redemption_probs,
        lambda_r=6.5, d_eth=10, d_sol=2
    )
    
    # Large batch (sensitivity analysis size)
    staking_levels_large = np.linspace(0.70, 1.0, 31, dtype=np.float64)
    te_results_large = calculate_tracking_error_vectorized(
        staking_levels_large, staking_levels_large,
        k_eth_eth, k_sol_sol, k_eth_sol,
        redemption_sizes, redemption_probs,
        lambda_r=6.5, d_eth=10, d_sol=2
    )
    ...
    benefits = calculate_net_benefits_vectorized(
        tracking_errors,
        staking_levels, staking_levels,
        eth_weight=0.1049, sol_weight=0.0387,
        eth_yield=0.035, sol_yield=0.06,
        eth_baseline=0.0, sol_baseline=0.0,
        redemption_sizes=redemption_sizes,
        redemption_probs=redemption_probs,
        lambda_r=6.5, d_eth=10, d_sol=2,
        current_td=0.002, cap_td=0.007
    )
