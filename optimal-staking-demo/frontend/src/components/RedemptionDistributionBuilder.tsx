...
import type { RedemptionDistributionItem } from "../types";
...
export const RedemptionDistributionBuilder: React.FC<
	RedemptionDistributionBuilderProps
> = ({
	distribution,
	onChange,
	...
	useEffect(() => {
		const handleMouseMove = (e: MouseEvent) => {
			...
			const newProbability = Math.max(
				0,
				Math.min(100, (relativeY / chartHeight) * 100),
			);
					...
					{distribution.map((item, index) => (
						...
						>
							<TextField
								...
								onChange={(e) =>
									updateSize(index, parseFloat(e.target.value) || 0)
								}
								InputProps={{
									endAdornment: (
			...
			>
				...
				<Stack spacing={0.5}>
					<Typography variant="caption" sx={{ color: "#D7E6FF" }}>
						• Total Probability:{" "}
						{(
							distribution.reduce((sum, item) => sum + item.probability, 0) *
							100
						).toFixed(1)}
						% (auto-normalized)
					</Typography>
					<Typography variant="caption" sx={{ color: "#D7E6FF" }}>
						...
						{(
							distribution.reduce(
								(sum, item) => sum + item.probability * item.size,
								0,
							) * 100
						).toFixed(2)}
						% of AUM
					...
					<Typography variant="caption" sx={{ color: "#D7E6FF" }}>
						• Chance of occurring:{" "}
						{((expectedRedemptionsPerYear / 365) * 100).toFixed(2)}% daily
					</Typography>
				</Stack>
			</Box>
		</Stack>
