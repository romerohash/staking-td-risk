...
const AssetStakingSection: React.FC<AssetStakingSectionProps> = ({
	...
	const unbondingInput = useDebouncedNumericInput({
		...
		onChange: (value) =>
			handleAssetStakingChange(asset, "unbonding_period_days", value),
		min: 0,
		decimalPlaces: 0,
	});
	...
	const stakingYieldInput = useDebouncedNumericInput({
		...
		onChange: (value) =>
			handleAssetStakingChange(asset, "annual_yield", value, true),
		min: 0,
		max: 20,
		decimalPlaces: 1,
	});
	...
	const baselineStakingInput = useDebouncedNumericInput({
		...
		onChange: (value) =>
			handleAssetStakingChange(asset, "baseline_staking_pct", value, true),
		min: 0,
		max: 100,
		decimalPlaces: 0,
	});
				...
				<Box>
					<Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
						<Typography variant="body2" color="text.secondary">
							Staking Percentage:{" "}
							{(parameters.staking[asset].staking_pct * 100).toFixed(0)}%
						</Typography>
						<InfoTooltip
...
export const ParameterForm: React.FC<ParameterFormProps> = ({
	...
	const [volatilityType, setVolatilityType] = useState<"daily" | "annual">(
		"daily",
	);
	...
	const hasExpandedSection = Object.values(expandedSections).some(
		(expanded) => expanded,
	);
	...
	const normalizeBenchmarkWeights = useCallback(
		(weights: BenchmarkWeights): BenchmarkWeights => {
			...
			const sumExceptLast = allButLast.reduce(
	...
	const normalizeRedemptionDistribution = useCallback(
		(items: RedemptionDistributionItem[]): RedemptionDistributionItem[] => {
		...
		<Stack spacing={3}>
			...
			<Box>
				<Button
					...
					onClick={() => {
						...
						const volatilitiesToApply =
							volatilityType === "annual"
								? Object.entries(defaultMarketParams.volatilities).reduce(
										(acc, [asset, vol]) => ({
											...
											[asset]:
												(vol ?? 0) *
												Math.sqrt(
													defaultMarketParams.trading_days_per_year ?? 252,
												),
										}),
						...
						const defaultDistribution = [
							...
							{ probability: 0.167, size: 0.1 },
							{ probability: 0.111, size: 0.2 },
							{ probability: 0.055, size: 0.3 },
						];
						const normalizedDistribution =
							normalizeRedemptionDistribution(defaultDistribution);
						...
						onChange({
							...
							market: {
								...
								trading_days_per_year:
									defaultMarketParams.trading_days_per_year,
							},
			...
			<Box sx={{ mb: 2 }}>
				<Button
					...
					startIcon={
						hasExpandedSection ? <ExpandLessIcon /> : <ExpandMoreIcon />
					}
					sx={glassStyles.buttonSubtle}
					fullWidth
				>
			...
			>
				...
				<Stack spacing={2}>
					...
					<TextField
						label="NAV"
						value={
							navInput.value === "" || Number.isNaN(parseFloat(navInput.value))
								? ""
			...
			>
				...
				<Stack spacing={2}>
					{Object.entries(marketParams.benchmark_weights).map(
						([asset, weight]) => {
									...
									<Slider
										...
										onChange={(_, value) => {
											...
											const normalizedWeights =
												normalizeBenchmarkWeights(newWeights);
											onChange({
												...parameters,
												market: {
													...marketParams,
					...
					<Typography variant="body2" color="text.secondary">
						Total:{" "}
						{(
							Object.values(marketParams.benchmark_weights).reduce(
								(sum, weight) => sum + weight,
								0,
							) * 100
						).toFixed(1)}
								...
								control={
									<Switch
										...
										onChange={(e) => {
											...
											const newVolatilities = Object.entries(
							...
							{(() => {
								...
								return (
									<TextField
										...
										onChange={(e) =>
											withinExcludedInput.onChange(e.target.value)
										}
										inputProps={{ min: -1, max: 1, step: 0.01 }}
										sx={glassStyles.input}
										fullWidth
			...
			>
				<Stack spacing={3}>
					...
					<Box>
						<Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
							<Typography variant="body2" color="text.secondary">
								Expected (Institutional) Redemptions per Year:{" "}
								{parameters.redemption.expected_redemptions_per_year.toFixed(1)}
							</Typography>
							<InfoTooltip
						...
						<Slider
							...
							onChange={(_, value) =>
								handleRedemptionChange(
									"expected_redemptions_per_year",
									value as number,
								)
							}
							min={0}
					...
					<RedemptionDistributionBuilder
						...
						expectedRedemptionsPerYear={
							parameters.redemption.expected_redemptions_per_year
						}
