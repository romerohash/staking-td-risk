...
function App() {
	...
	const documentItems = [
		{
			name: "Analytical Tracking Error Formula",
			path: "analytical-tracking-error-formula.md",
		},
		{
			name: "Tracking Error Time Window Invariance",
			path: "tracking-error-time-window-invariance.md",
		},
		{
			name: "Two-Asset Tracking Error Formula",
			path: "two-asset-tracking-error-formula.md",
		},
		{
			name: "Staking Benefit Formulation",
			path: "staking-benefit-formulation.md",
		},
		{
			name: "Two-Asset Model Parameters",
	...
	const handleCalculate = useCallback(async () => {
		...
		try {
			...
			if (volatilityType === "annual" && parameters.market) {
				const sqrtTradingDays = Math.sqrt(
					parameters.market.trading_days_per_year || 252,
				);
				requestParams = {
					...parameters,
					market: {
	...
	useEffect(() => {
		if (autoFitToOptimal && results?.optimal_staking_levels) {
			...
			if (
				Math.abs(currentEth - optimalEth) > tolerance ||
		...
		autoFitToOptimal,
		results?.optimal_staking_levels,
		parameters.staking.eth.staking_pct,
		parameters.staking.sol.staking_pct,
	]);
					...
					>
						<Box
							...
							sx={{
								...
								mr: 2,
								filter:
									"brightness(0) saturate(100%) invert(77%) sepia(23%) saturate(1076%) hue-rotate(175deg) brightness(100%) contrast(101%)",
								opacity: 0.9,
								transition: "all 0.3s ease",
								"&:hover": {
									opacity: 1,
									filter:
										"brightness(0) saturate(100%) invert(77%) sepia(23%) saturate(1076%) hue-rotate(175deg) brightness(110%) contrast(101%)",
								},
							}}
						/>
				...
				>
					<Box sx={{ p: 3, pt: 11, overflowY: "auto", flex: 1, pb: 10 }}>
						...
						<Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
							Adjust parameters to analyze tracking error / net benefits
							trade-off
						</Typography>
					...
					<Box
						sx={{
							...
							p: 1.5,
							background:
								"linear-gradient(to top, rgba(29, 41, 57, 0.95) 0%, rgba(29, 41, 57, 0.85) 70%, transparent 100%)",
							backdropFilter: "blur(10px)",
							WebkitBackdropFilter: "blur(10px)",
					...
					>
						...
						>
							...
							<Tooltip
								title="Automatically recalculate when parameters change"
								placement="top"
							>
								<FormControlLabel
									control={
