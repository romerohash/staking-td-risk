...
class TwoPointDistribution:
    ...
    def describe(self) -> str:
        ...
        return (f"{self.r1:.1%} with prob {self.p:.1f}, "
...
def analytical_te(lambda_redemptions: float,
                 episode_days: float,
                 base_k: float,
                 staking_pct: float,
                 distribution: TwoPointDistribution) -> float:
    """
...
def sensitivity_analysis_staking(base_k: float,
    ...
    for staking_pct in staking_pcts:
        te = analytical_te(lambda_redemptions, episode_days, base_k, 
                          staking_pct, distribution)
        te_values.append(te)
    
    return staking_pcts.tolist(), te_values


def sensitivity_analysis_distribution(base_k: float,
                                    lambda_redemptions: float,
                                    episode_days: float,
                                    staking_pct: float,
...
def main():
    ...
    for staking in [0.7, 0.8, 0.9, 0.95]:
        ...
        print(f"  {staking:.0%} staking (τ={threshold:.2f}): "
