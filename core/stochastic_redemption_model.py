...
class VarianceParameters:
    ...
    @classmethod
    def from_market_config(cls, market: MarketConfig, staking_pct: float) -> VarianceParameters:
...
class EnhancedStochasticModel:
    ...
    def expected_variance(self) -> float:
        ...
        if isinstance(self.redemption_dist, stats.rv_discrete):
            ...
            if hasattr(self.redemption_dist, 'xk') and hasattr(self.redemption_dist, 'pk'):
    ...
    def variance_of_variance(self) -> float:
        ...
        if isinstance(self.redemption_dist, stats.rv_discrete):
            ...
            def integrand(r):
                if r <= threshold:
                    return 0.0
                return self.redemption_dist.pdf(r) * (base_k * (r - threshold)**2)**2
    ...
    def simulate_tracking_error(self, n_simulations: int = 10000) -> NDArray:
        ...
        for i in range(n_simulations):
            ...
            if n_redemptions > 0:
                ...
                total_var = sum(
                    self.variance_params.variance(r) for r in redemptions
                )
    ...
    def analytical_derivatives(self) -> Dict[str, float]:
        ...
        # ∂TE/∂λ = TE/(2λ)
        d_lambda = te / (2 * self.lambda_redemptions) if self.lambda_redemptions > 0 else 0
        ...
        new_model = EnhancedStochasticModel(
            self.lambda_redemptions,
            self.redemption_dist,
            new_params,
            self.episode_days
        )
...
def validate_against_discrete(market: MarketConfig, staking_pct: float,
                            schedule: List[Tuple[float, int, int]]) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        'validation': {
            ...
            'variance_calculation_error': abs(expected_var_numerical - expected_var_discrete),
            'analytical_in_mc_range': te_ci[0] <= mc_te_mean <= te_ci[1]
        }
    }


def sensitivity_analysis_enhanced(market: MarketConfig, base_staking: float,
                                lambda_range: Tuple[float, float],
                                mean_range: Tuple[float, float]) -> Dict[str, Any]:
    """
    ...
    if center_te > 0:
        ...
        lambda_deriv = (te_grid[center_i+1, center_j] - te_grid[center_i-1, center_j]) / (2*d_lambda)
        lambda_elasticity = lambda_deriv * lambdas[center_i] / center_te
        
        # Mean elasticity
        d_mean = means[1] - means[0]
        mean_deriv = (te_grid[center_i, center_j+1] - te_grid[center_i, center_j-1]) / (2*d_mean)
    ...
    return {
        ...
        'te_grid': te_grid,
        'elasticities': {
            'lambda': lambda_elasticity,
            'mean': mean_elasticity
        },
        'threshold_effects': {
...
def demonstrate_non_constant_k():
    ...
    for r in redemptions:
        ...
        print(f"{r:8.1%}   {variance:9.6f}  {naive_k:9.6f}  {actual_k:9.6f}  "
...
def main():
    ...
    print(f"  95% CI: [{validation['analytical_results']['te_ci'][0]:.4%}, "
    ...
    print(f"\nThreshold Effects:")
    print(f"  Average TE below threshold: {sensitivity['threshold_effects']['below_threshold_te']:.4%}")
    print(f"  Average TE above threshold: {sensitivity['threshold_effects']['above_threshold_te']:.4%}")
