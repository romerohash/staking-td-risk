...
class StakingYieldParameters:
    ...
    annual_staking_yield: float = Field(
        0.04, description='Annual staking yield (e.g., 4%)'
    )
    baseline_staking_percentage: float = Field(
        0.70, description='Reference staking level'
    )
...
class TwoAssetTrackingError:
    ...
    def net_benefit_analysis(self) -> Dict[str, float]:
        ...
        eth_benefit_baseline = (
            ...
            * max(0, self.staking.eth_staking_pct - baseline)
            * self.yield_params.annual_staking_yield
        )
        sol_benefit_baseline = (
            self.benchmark.sol_weight
            * max(0, self.staking.sol_staking_pct - baseline)
            * self.yield_params.annual_staking_yield
        )
