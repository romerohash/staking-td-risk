...
class StakingYieldParameters:
    ...
    annual_staking_yield: float = Field(0.04, description="Annual staking yield (e.g., 4%)")
    baseline_staking_percentage: float = Field(0.70, description="Reference staking level")
    
    @field_validator('annual_staking_yield', 'baseline_staking_percentage')
...
class TwoAssetTrackingError:
    ...
    def net_benefit_analysis(self) -> Dict[str, float]:
        ...
        baseline = self.yield_params.baseline_staking_percentage
        eth_benefit_baseline = self.benchmark.eth_weight * max(0, self.staking.eth_staking_pct - baseline) * self.yield_params.annual_staking_yield
        sol_benefit_baseline = self.benchmark.sol_weight * max(0, self.staking.sol_staking_pct - baseline) * self.yield_params.annual_staking_yield
        ...
        eth_benefit_marginal = self.benchmark.eth_weight * self.yield_params.annual_staking_yield * eth_episode_fraction * exp_excess_eth
        sol_benefit_marginal = self.benchmark.sol_weight * self.yield_params.annual_staking_yield * sol_episode_fraction * exp_excess_sol
        ...
        # Net benefit
        net_benefit = total_benefit + expected_shortfall
...
def main():
    ...
    market = MarketParameters()
    redemption = RedemptionDistribution()
    yield_params = StakingYieldParameters(annual_staking_yield=0.05)
    ...
    print(f"\nNET BENEFIT ANALYSIS:")
    print("-" * 80)
    print(f"Staking Yield Benefits:")
    print(f"  ETH:")
