...
def compute_k_components(market_cov: NDArray[np.float64],
                        eth_idx: int = 1,
                        sol_idx: int = 3) -> Dict[str, float]:
    """
...
def variance_full_period(r: float, config: TwoAssetConfig, k_components: Dict[str, float]) -> float:
    """
    ...
    # Calculate variance
    variance = (k_eth_eth * excess_eth**2 +
...
def variance_partial_period(r: float, config: TwoAssetConfig, k_components: Dict[str, float]) -> float:
    """
...
def expected_values_discrete(redemption_dist: stats.rv_discrete,
                           config: TwoAssetConfig,
                           k_components: Dict[str, float]) -> Dict[str, float]:
    """
...
def analytical_tracking_error_two_asset(config: TwoAssetConfig,
                                      k_components: Dict[str, float],
                                      expectations: Dict[str, float]) -> float:
    """
...
def decompose_tracking_error(config: TwoAssetConfig,
                           k_components: Dict[str, float],
                           expectations: Dict[str, float]) -> Dict[str, float]:
    """
    ...
    eth_contribution = config.lambda_redemptions * d_long * k_eth_eth * expectations['exp_eth_squared']
    sol_contribution = config.lambda_redemptions * d_short * k_sol_sol * expectations['exp_sol_squared']
    cross_contribution = config.lambda_redemptions * d_short * 2 * k_eth_sol * expectations['exp_cross_term']
    ...
    te_eth_only = sqrt(config.lambda_redemptions * d_long * k_eth_eth * expectations['exp_eth_squared'])
    te_sol_only = sqrt(config.lambda_redemptions * d_short * k_sol_sol * expectations['exp_sol_squared'])
    
    return {
        'total_te': total_te,
        'te_eth_only': te_eth_only,
        'te_sol_only': te_sol_only,
        'eth_contribution_pct': eth_contribution / total_variance * 100 if total_variance > 0 else 0,
        'sol_contribution_pct': sol_contribution / total_variance * 100 if total_variance > 0 else 0,
        'cross_contribution_pct': cross_contribution / total_variance * 100 if total_variance > 0 else 0,
        'k_eth_eth': k_eth_eth,
        'k_sol_sol': k_sol_sol,
        'k_eth_sol': k_eth_sol,
...
def main():
    ...
    print(f"\nConfiguration:")
    print(f"  ETH: {config.eth_weight:.2%} weight, {config.eth_staking_pct:.0%} staked, {config.eth_unbonding_days} day unbonding")
    print(f"  SOL: {config.sol_weight:.2%} weight, {config.sol_staking_pct:.0%} staked, {config.sol_unbonding_days} day unbonding")
    ...
    print(f"  Independence approx: {(decomp['te_eth_only']**2 + decomp['te_sol_only']**2)**0.5:.4%}")
    ...
    for r in values:
        ...
        var_days = config.min_unbonding * var_full + (config.max_unbonding - config.min_unbonding) * var_partial
