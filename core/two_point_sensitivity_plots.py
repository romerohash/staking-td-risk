...
from core.two_point_distribution_analysis import (
    ...
    calculate_base_k,
    sensitivity_analysis_staking,
    sensitivity_analysis_distribution,
)
...
def plot_staking_sensitivity():
    ...
    plt.title(
        'Tracking Error Sensitivity to Staking Level\n(Two-Point Redemption Distributions)',
        fontsize=14,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        'images/staking_sensitivity_two_point.png', dpi=150, bbox_inches='tight'
    )
    plt.close()
...
def plot_distribution_heatmap():
    ...
    fig.suptitle(
        f'Tracking Error Heatmaps for Two-Point Distributions\n'
    ...
    plt.tight_layout()
    plt.savefig(
        'images/distribution_heatmap_two_point.png', dpi=150, bbox_inches='tight'
    )
    plt.close()
...
def plot_threshold_effects():
    ...
    bars1 = ax2.bar(
        x - width / 2, contributions_r1, width, label=f'r1={r1:.0%} (p={p:.1f})'
    )
    bars2 = ax2.bar(
        x + width / 2, contributions_r2, width, label=f'r2={r2:.0%} (p={1 - p:.1f})'
    )
    ...
    plt.suptitle(
        f'Threshold Effects for Two-Point Distribution\n'
        f'({r1:.0%} with p={p}, {r2:.0%} with p={1 - p})',
        fontsize=14,
    )
...
def main():
    ...
    print(
        "3. Threshold effects create 'safe zones' where small redemptions don't impact TE"
    )
